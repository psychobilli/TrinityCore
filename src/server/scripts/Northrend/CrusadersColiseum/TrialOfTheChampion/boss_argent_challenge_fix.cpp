...
{
    argent_challenge_baseAI(Creature* creature) : BossAI(creature, BOSS_ARGENT_CHALLENGE)
    {
        Initialize();
    ...
    {
        if (instance->GetBossState(BOSS_ARGENT_CHALLENGE) != SPECIAL &&
            instance->GetBossState(BOSS_ARGENT_CHALLENGE) != DONE)
    ...
    {
        if (!me->HasAura(SPELL_SHIELD) && instance->GetBossState(BOSS_ARGENT_CHALLENGE) != SPECIAL)
...
public:
    ...
    struct boss_eadricAI : public argent_challenge_baseAI
    {
        ...
        {
            if (instance->GetBossState(BOSS_ARGENT_CHALLENGE) == DONE)
            {
                damage = 0;
                return;
            }
            ...
            if (damage >= me->GetHealth())
            {
                ...
                if (instance->GetBossState(BOSS_ARGENT_CHALLENGE) != SPECIAL)
                {
                    instance->SetBossState(BOSS_ARGENT_CHALLENGE, SPECIAL);
                    events.Reset();
                    Talk(SAY_DEFEATED_E);
                }
            }
        ...
        void JustEngagedWith(Unit* who) override
        {
            ...
            Milliseconds time = (Milliseconds)urand(7000, 15000);
            events.ScheduleEvent(EVENT_RADIANCE, time);
        ...
        void UpdateAI(uint32 uiDiff) override
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_RADIANCE:
                    ...
                    events.ScheduleEvent(EVENT_HAMMER_OF_JUSTICE, time);
                    break;
                case EVENT_HAMMER_OF_JUSTICE:
                    ...
                    time = (Milliseconds)urand(7000, 15000);
                    events.ScheduleEvent(EVENT_RADIANCE, time);
                    break;
                case EVENT_CHALLENGE_DONE:
                    ...
                    DoCastAOE(SPELL_EADRIC_ACHIEVEMENT, true);
...
public:
    ...
    struct boss_paletressAI : public argent_challenge_baseAI
    {
        ...
        void JustEngagedWith(Unit* who) override
        {
            ...
            Milliseconds time = (Milliseconds)urand(9000, 12000);
            events.ScheduleEvent(EVENT_HOLY_FIRE, time);
            time = (Milliseconds)urand(15000, 17000);
            events.ScheduleEvent(EVENT_RENEW, time);
...
public:
    ...
    struct npc_memoryAI : public ScriptedAI
    {
        npc_memoryAI(Creature* creature) : ScriptedAI(creature)
        {
            ...
            Milliseconds time = (Milliseconds)urand(14000, 16000);
            _events.ScheduleEvent(EVENT_OLD_WOUNDS, time);
            _events.ScheduleEvent(EVENT_SHADOWS_PAST, 8s);
            time = (Milliseconds)urand(10000, 13000);
            _events.ScheduleEvent(EVENT_WAKING_NIGHTMARE, time);
        }
...
public:
    ...
    class spell_eadric_hammer_of_righteous_SpellScript : public SpellScript
    {
        ...
        void HandleDamage(SpellMissInfo /*info*/)
        {
